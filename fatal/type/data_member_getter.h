...
class FATAL_HIDE_SYMBOL data_member_getter {
  using impl = Impl;

  struct FATAL_HIDE_SYMBOL has_impl {
...
public:
  ...
  template <typename Owner>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  static constexpr typename std::decay<type<Owner>>::type copy(
    Owner &&owner
  ) {
  ...
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  static void set(Owner &&owner, Value &&value) {
    return impl::set(std::forward<Owner>(owner), std::forward<Value>(value));
  }

  template <typename Owner>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  static constexpr reference<Owner> ref(Owner &&owner) {
  ...
  template <typename Owner>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  static constexpr pointer<Owner> ptr(Owner &owner) {
...
private:
  template <typename Owner>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  static constexpr pointer<Owner> t(std::true_type, Owner &owner) {
    return ptr(owner);
  }

  template <typename Owner>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  static constexpr pointer<Owner> t(std::false_type, Owner &) {
    return nullptr;
  }
...
public:
  template <typename Owner>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  static constexpr pointer<Owner> try_get(Owner &owner) {
    return t(has<Owner>(), owner);
  }
};

template <typename Getter>
struct FATAL_HIDE_SYMBOL data_member_copier {
  template <typename Owner>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
...
template <typename Getter>
struct FATAL_HIDE_SYMBOL data_member_setter {
  template <typename Owner, typename Value>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  void operator ()(Owner &&owner, Value &&value) const {
    return Getter::set(std::forward<Owner>(owner), std::forward<Value>(value));
  }
};

template <typename Getter>
struct FATAL_HIDE_SYMBOL data_member_referencer {
  template <typename Owner>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  constexpr typename Getter::template reference<Owner> operator ()(
    Owner &&owner
  ) const {
    static_assert(
...
template <typename Getter>
struct FATAL_HIDE_SYMBOL data_member_pointer {
  template <typename Owner>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  constexpr typename Getter::template pointer<Owner> operator ()(
    Owner &owner
  ) const {
    static_assert(
...
template <typename Getter>
struct FATAL_HIDE_SYMBOL data_member_try_getter {
  template <typename Owner>
  FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
  constexpr typename Getter::template pointer<Owner> operator ()(
    Owner &owner
...
#define FATAL_IMPL_DATA_MEMBER_GETTER(Class, Impl, ...) \
  struct FATAL_HIDE_SYMBOL Impl { \
    ...
    template <typename Owner> \
    FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL \
    static constexpr typename reference<Owner>::ref_impl ref( \
      Owner &&owner \
    ) { \
 ...
 */
template <typename...>
class FATAL_HIDE_SYMBOL chained_data_member_getter;

template <typename OuterGetter, typename... Getters>
class FATAL_HIDE_SYMBOL chained_data_member_getter<OuterGetter, Getters...> {
...
public:
  ...
  struct FATAL_HIDE_SYMBOL ref_getter {
    template <typename Owner>
    FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
    reference<Owner> operator ()(Owner &&owner) const {
  ...
  struct FATAL_HIDE_SYMBOL ptr_getter {
    template <typename Owner>
    FATAL_ALWAYS_INLINE FATAL_HIDE_SYMBOL
