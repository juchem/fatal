...
#define TEST_TRIE_FIND(Expected, Haystack, Needle) \
  do { \
    ...
    bool const actual = trie_exact_match<Haystack>(needle.begin(), needle.end()); \
    FATAL_EXPECT_EQ(Expected, actual); \
  } while (false)

FATAL_TEST(trie_exact_match, exact) {
  TEST_TRIE_FIND(true, hs_tree, h);
  ...
  TEST_TRIE_FIND(false, hs_tree, hin);
  TEST_TRIE_FIND(true, hs_tree, hint);
  TEST_TRIE_FIND(true, hs_tree, ho);
  TEST_TRIE_FIND(false, hs_tree, hottoo);
  TEST_TRIE_FIND(false, hs_tree, hoT);
  TEST_TRIE_FIND(false, hs_tree, hoTtoo);
